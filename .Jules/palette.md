...
## 2024-05-15 - Revealing Interactive Elements
**Learning:** Relying solely on `group-hover:opacity-100` for custom list actions (like task completion checkboxes) completely hides them from keyboard users.
**Action:** When hiding interactive elements by default, always pair `group-hover:opacity-100` with `group-focus-within:opacity-100` so that navigating via keyboard (tabbing into the container) makes the actionable elements visible.
